...
The generic denied response for `/haven toggleop` is configurable at `haven.toggleop-denied` in `messages.yml`. Successful OP-toggle responses are configurable at `haven.toggleop-enabled` and `haven.toggleop-disabled`.
