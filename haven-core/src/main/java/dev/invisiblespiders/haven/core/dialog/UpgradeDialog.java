...
public class UpgradeDialog {
    ...
    public void open(Player player, UpgradeViewRequest request) {
        ...
        List<DialogBody> body = buildBodyComponents(player, request, definitions, providers).stream()
                .map(c -> (DialogBody) DialogBody.plainMessage(c))
                .toList();
        ...
        Dialog dialog = Dialog.create(factory -> factory.empty()
                .base(DialogBase.builder(CoreText.config(config.getUpgrades(), "ui.title", "<gold>Upgrades"))
                        ...
                        .build())
                .type(DialogType.multiAction(List.of(), close, 1)));
        player.showDialog(dialog);
    }

    List<Component> buildBodyComponents(Player player, UpgradeViewRequest request,
                                        List<UpgradeDefinition> definitions,
                                        Map<String, UpgradeProvider> providers) {
        if (definitions.isEmpty()) {
            return List.of(Component.text("No upgrades available.", NamedTextColor.GRAY));
        }
        List<Component> rows = new ArrayList<>();
        String currentCategoryId = null;
        for (UpgradeDefinition definition : definitions) {
            String catId = definition.category().id();
            if (!catId.equals(currentCategoryId)) {
                currentCategoryId = catId;
                rows.add(CoreText.deserialize("<gold><bold>" + definition.category().displayName()));
            }
            rows.add(entryComponent(player, request, definition, providers.get(definition.providerId())));
        }
        return rows;
    }

    private Component entryComponent(Player player, UpgradeViewRequest request,
                                     UpgradeDefinition definition, UpgradeProvider provider) {
        ...
        Component label;
        if (maxed) {
            label = CoreText.deserialize(
                    CoreText.configString(config.getUpgrades(), "ui.buttons.maxed", "<gray>Maxed")
                    + " <white>" + levelName);
        } else {
            label = CoreText.deserialize("<white>  " + levelName);
        }

        String providerName = provider == null ? definition.providerId() : provider.displayName();
        label = label.hoverEvent(HoverEvent.showText(Component.text(
                providerName + " / " + definition.id()
                + (locked ? " (locked)" : maxed ? " (maxed)" : ""))));

        if (!locked && !maxed) {
            final int capturedLevel = currentLevel;
            final String capturedLevelName = levelName;
            label = label.clickEvent(ClickEvent.callback(audience -> {
                if (audience instanceof Player p) {
                    openConfirm(p, request, definition, capturedLevel, capturedLevelName);
                }
            }, multiUseOpts()));
        }
        return label;
